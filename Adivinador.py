...
def arma_nuevo_numero(lista_picas, lista_fallas):
    ...
    for caracter in lista_fallas_temp:
        if 'F' in lista_picas_temp:
            lista_picas_temp[lista_picas_temp.index('*')] = caracter
    ...
    while 'F' in lista_picas_temp:
        lista_picas_temp[lista_picas_temp.index('*')] = str(random.randint(0,9))

    print(lista_picas)
    return int(''.join(lista_picas_temp))
    #Para el que se anime a incluirle:
    #1. Controles para las repuestas
	#2. Reemplazar el caracter 'F' en la lista de picas (Checked)
	#3. Controlar las sugerencias utilizando permutaciones
